import { analyzeDimensionPerformance } from "@/lib/dimensionPerformance";
...
export default function RegionPerformance({ rows }: RegionPerformanceProps) {
  const result = analyzeDimensionPerformance({
    rows,
    dimensionLabel: "Region",
    dimensionColumnCandidates: REGION_COLUMNS,
  });
      ...
      <section className="mt-6 w-full max-w-6xl rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-left shadow-sm">
        ...
        <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">
          <p className="font-medium text-slate-800">
            Region Performance is not available for this sheet.
          </p>
          ...
          <p className="mt-1">
            Please select a sheet that contains region level financial data.
          </p>
        </div>
      </section>
    );
